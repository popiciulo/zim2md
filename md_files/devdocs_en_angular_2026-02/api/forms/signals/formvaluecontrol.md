...
A contract for a form control that edits a [`FieldTree`](fieldtree) of type `TValue`. Any component that implements this contract can be used with the [`Field`](field) directive.
...
The value is the only required property in this contract. A component that wants to integrate with the [`Field`](field) directive via this contract, *must* provide a `model()` that will be kept in sync with the value of the bound [`FieldTree`](fieldtree).

### checked

`undefined`

The implementing component *must not* define a `checked` property. This is reserved for components that want to integrate with the [`Field`](field) directive as a checkbox.
