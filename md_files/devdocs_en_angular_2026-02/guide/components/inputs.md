...
The [`input`](../../api/core/input) function has special meaning to the Angular compiler. **You can exclusively call [`input`](../../api/core/input) in component and directive property initializers.**
...
The [`input`](../../api/core/input) function returns an [`InputSignal`](../../api/core/inputsignal). You can read the value by calling the signal:
...
Signals created by the [`input`](../../api/core/input) function are read-only.

## Required inputs

You can declare that an input is [`required`](../../api/forms/signals/required) by calling [`input.required`](../../api/core/input#required) instead of [`input`](../../api/core/input):
...
The [`input`](../../api/core/input) function accepts a config object as a second parameter that lets you change the way that input works.
...
In other respects, model inputs work similarly to standard inputs. You can read the value by calling the signal function, including in reactive contexts like [`computed`](../../api/core/computed) and [`effect`](../../api/core/effect).

See [Two-way binding](../templates/two-way-binding) for more details on two-way binding in templates.
...
When you declare a model input in a component or directive, Angular automatically creates a corresponding [output](outputs) for that model. The output's name is the model input's name suffixed with "Change".
...
See [Custom events with outputs](outputs) for more details on outputs.

### Customizing model inputs

You can mark a model input as required or provide an alias in the same way as a [standard input](inputs).
...
**TIP:** While the Angular team recommends using the signal-based [`input`](../../api/core/input) function for new projects, the original decorator-based [`@Input`](../../api/core/input) API remains fully supported.
...
You can specify the [`required`](../../api/forms/signals/required) option to enforce that a given input must always have a value.
