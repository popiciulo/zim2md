...
There are two main ways to render a component programmatically: in a template using [`NgComponentOutlet`](../../api/common/ngcomponentoutlet), or in your TypeScript code using [`ViewContainerRef`](../../api/core/viewcontainerref).

**HELPFUL:** for lazy-loading use-cases (for example if you want to delay loading of a heavy component), consider using the built-in [`@defer` feature](../templates/defer) instead. The `@defer` feature allows the code of any components, directives, and pipes inside the `@defer` block to be extracted into separate JavaScript chunks automatically and loaded only when necessary, based on the configured triggers.

## Using NgComponentOutlet

[`NgComponentOutlet`](../../api/common/ngcomponentoutlet) is a structural directive that dynamically renders a given component in a template.
...
See the [NgComponentOutlet API reference](../../api/common/ngcomponentoutlet) for more information on the directive's capabilities.

## Using ViewContainerRef

A **view container** is a node in Angular's component tree that can contain content. Any component or directive can inject [`ViewContainerRef`](../../api/core/viewcontainerref) to get a reference to a view container corresponding to that component or directive's location in the DOM.

You can use the [`createComponent`](../../api/core/createcomponent)method on [`ViewContainerRef`](../../api/core/viewcontainerref) to dynamically create and render a component. When you create a new component with a [`ViewContainerRef`](../../api/core/viewcontainerref), Angular appends it into the DOM as the next sibling of the component or directive that injected the [`ViewContainerRef`](../../api/core/viewcontainerref).
...
**HELPFUL:** if you want to lazy-load some components, you may consider using the built-in [`@defer` feature](../templates/defer) instead.

If your use-case is not covered by the `@defer` feature, you can use either [`NgComponentOutlet`](../../api/common/ngcomponentoutlet) or [`ViewContainerRef`](../../api/core/viewcontainerref) with a standard JavaScript [dynamic import](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Operators/import).
...
To simplify this, both [`createComponent`](../../api/core/createcomponent) and [`ViewContainerRef.createComponent`](../../api/core/viewcontainerref#createComponent) support passing a `bindings` array with helpers like [`inputBinding()`](../../api/core/inputbinding), [`outputBinding()`](../../api/core/outputbinding), and [`twoWayBinding()`](../../api/core/twowaybinding) to configure inputs and outputs up front. You can also specify a `directives` array to apply any host directives. This enables creating components programmatically with template-like bindings in a single, declarative call.

### Host view using ViewContainerRef.createComponent

[`ViewContainerRef.createComponent`](../../api/core/viewcontainerref#createComponent) creates a component and automatically inserts its host view and host element into the container’s view hierarchy at the container’s location. Use this when the dynamic component should become part of the container’s logical and visual structure (for example, adding list items or inline UI).

By contrast, the standalone [`createComponent`](../../api/core/createcomponent) API does not attach the new component to any existing view or DOM location — it returns a [`ComponentRef`](../../api/core/componentref) and gives you explicit control over where to place the component’s host element.
