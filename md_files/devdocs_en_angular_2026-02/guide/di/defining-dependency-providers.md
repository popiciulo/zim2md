...
1. **Automatic provision** - Using `providedIn` in the [`@Injectable`](../../api/core/injectable) decorator or by providing a factory in the [`InjectionToken`](../../api/core/injectiontoken) configuration
2. **Manual provision** - Using the `providers` array in components, directives, routes, or application config

In the [previous guide](creating-and-using-services), you learned how to create services using `providedIn: 'root'`, which handles most common use cases. This guide explores additional patterns for both automatic and manual provider configuration.

## Automatic provision for non-class dependencies

While the [`@Injectable`](../../api/core/injectable) decorator with `providedIn: 'root'` works great for services (classes), you might need to provide other types of values globally - like configuration objects, functions, or primitive values. Angular provides [`InjectionToken`](../../api/core/injectiontoken) for this purpose.

### What is an InjectionToken?

An [`InjectionToken`](../../api/core/injectiontoken) is an object that Angular's dependency injection system uses to uniquely identify values for injection. Think of it as a special key that lets you store and retrieve any type of value in Angular's DI system:
...
An [`InjectionToken`](../../api/core/injectiontoken) that has a `factory` results in `providedIn: 'root'` by default (but can be overidden via the `providedIn` prop).
...
Use application-level providers in [`bootstrapApplication`](../../api/platform-browser/bootstrapapplication) when:
...
- You're using Angular's `provideSomething` pattern (e.g., [`provideRouter`](../../api/router/providerouter), [`provideHttpClient`](../../api/common/http/providehttpclient))
...
5. **Consistency** - Aligns with Angular's own patterns ([`provideRouter`](../../api/router/providerouter), [`provideHttpClient`](../../api/common/http/providehttpclient), etc.)
