...
Signals are useful because they notify interested consumers when they change. An **effect** is an operation that runs whenever one or more signal values change. You can create an effect with the [`effect`](../api/core/effect) function:
...
Effects are rarely needed in most application code, but may be useful in specific circumstances. Here are some examples of situations where an [`effect`](../api/core/effect) might be a good solution:
...
Instead, use [`computed`](../api/core/computed) signals to model state that depends on other state.

### Injection context

By default, you can only create an [`effect()`](../api/core/effect) within an [injection context](di/dependency-injection-context) (where you have access to the [`inject`](../api/core/inject) function). The easiest way to satisfy this requirement is to call [`effect`](../api/core/effect) within a component, directive, or service `constructor`:
...
To create an effect outside the constructor, you can pass an [`Injector`](../api/core/injector) to [`effect`](../api/core/effect) via its options:
