...
Bindings that change over time should read values from [signals](../signals). Angular tracks the signals read in the template, and updates the rendered page when those signal values change.
...
For more details, see the [Signals guide](../signals).
...
You can bind to properties on an HTML element's DOM instance, a [component](../components/anatomy-of-components) instance, or a [directive](../directives) instance.
...
Some ARIA features expose DOM properties or directive inputs that accept structured values (such as element references). Use standard property bindings for those cases. See the [accessibility guide](https://angular.dev/best-practices/a11y#aria-attributes-and-properties) for examples and additional guidance.
