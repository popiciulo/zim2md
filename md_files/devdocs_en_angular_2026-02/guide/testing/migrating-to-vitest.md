...
| Option | Description |
| --- | --- |
| `--project <name>` | Specify the project to refactor in a multi-project workspace.   Example: `--project=my-lib` |
| `--include <path>` | Refactor only a specific file or directory.   Example: `--include=src/app/app.component.spec.ts` |
| `--file-suffix <suffix>` | Specify a different file suffix for test files.   Example: `--file-suffix=.test.ts` |
...
**1. Direct path:** Provide a direct path to a Vitest configuration file in your `angular.json`:
...
**2. Automatic search for base configuration:** If you set `runnerConfig` to `true`, the builder will automatically search for a shared `vitest-base.config.*` file in your project and workspace roots.
